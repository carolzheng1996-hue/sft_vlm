...
from api_sft.common import iter_jsonl
from api_sft.compact_json import write_json, write_jsonl
from api_sft.trajectory_exporters import _trl_record
...
def export_trajectory_datasets(verified_path: Path, output_dir: Path) -> dict[str, Any]:
    records = list(iter_jsonl(verified_path)) if verified_path.exists() else []
    stale = [record.get("id") for record in records if record.get("format_version") != TRAJECTORY_FORMAT_VERSION]
    if stale:
        raise RuntimeError(
            "Cannot export non-single-candidate trajectory formats: "
    ...
    modality_counts = Counter(
        (record.get("question") or {}).get("input_mode") for record in records
    )
    tool_counts = Counter(
        event.get("name")
    ...
    summary = {
        ...
        "source_format": TRAJECTORY_FORMAT_VERSION,
        "format": "trl_conversational_tool_sft_v1",
        "task_counts": {key: value for key, value in task_counts.items() if key is not None},
        "input_mode_counts": {
            key: value for key, value in modality_counts.items() if key is not None
        },
        "successful_tool_counts": {
            key: value for key, value in tool_counts.items() if key is not None
        },
        "files": {
