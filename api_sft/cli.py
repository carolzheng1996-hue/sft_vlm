...
from .questions import build_question_specs, generate_questions, refresh_question_records, rewrite_questions
from .scenarios import GENERATOR_VERSION, generate_scenarios, migrate_scenarios
...
def archive_trajectory_run(p: dict[str, Path]) -> Path | None:
...
def do_questions(config: dict[str,Any], p: dict[str,Path], workspace: Path, model_config: dict[str,Any], limit: int|None=None, resume: bool=False) -> None:
    ...
    rows=generate_questions(p["question_specs"],p["questions"],p["question_rejected"],cfg,writer_path,resume,limit,int(generation.get("question_max_attempts",2)))
    print(json.dumps({"questions":len(rows),"output":str(p["questions"]),"rejected":str(p["question_rejected"]),"coverage":str(p["coverage"])},ensure_ascii=False))
...
def main() -> None:
    ...
    s=sub.add_parser("generate-questions"); s.add_argument("--limit",type=int); s.add_argument("--resume",action="store_true")
    ...
    elif args.command=="generate-questions": do_questions(config,p,workspace,models["question"],args.limit,args.resume)
    elif args.command=="refresh-question-records":
        rows=refresh_question_records(p["question_specs"],p["questions"]); print(json.dumps({"questions":len(rows),"output":str(p["questions"]),"api_calls":0},ensure_ascii=False))
    elif args.command=="rewrite-questions":
        print("WARNING: rewrite-questions 已弃用；当前命令等价于 generate-questions。")
        generation=config.get("generation",{}); writer_path=resolve_path(generation.get("question_writer_prompt","api_sft/prompts/question_writer_system.txt"),workspace)
        rewrite_questions(p["question_specs"],p["questions"],p["question_rejected"],models["question"],writer_path,args.resume,args.limit,int(generation.get("question_max_attempts",2)))
